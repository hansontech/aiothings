...
def storeGetObject(objectName):
    ...
    try:
        ...
        data = client.get_object(Bucket = os.environ['S3_BUCKET'], Key = 'private/' + os.environ['OWNER_ID'] + '/' + objectName)
...
def storePutObject(objectName, data):
    try:
        ...
        client.put_object(Bucket = os.environ['S3_BUCKET'], Key = 'private/' + os.environ['OWNER_ID'] + '/' + objectName, Body = dataString)
    except Exception as err:
        print(err)
