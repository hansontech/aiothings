...
export default {
  ...
  async created () {
    ...
    if (typeof this.copiedService !== 'undefined') {
      ...
      this.mservice.UserId = this.$store.getters.userId
      if (this.mservice.CodeEntryType === 'zip') {
        this.duplicateZipFile()
      } else {
  ...
  methods: {
    ...
    duplicateZipFile () {
      ...
      // read zip file from cloud storage
      this.isZipDownloading = true
      let that = this
      Storage.get(this.mservice.CodeFileName)
      .then(async function (resultUrl) {
        // console.log(result)
        // save as another zip file to cloud storage
        let dataResponse = await fetch(resultUrl)
        let blob = await dataResponse.blob()
        let fileName = that.mservice.UserId + '_' + that.mservice.ServiceName + '.zip'
        Storage.put(fileName, blob, {
            contentType: 'application/zip'
        })
        .then(result => {
          // console.log(result)
          // assign the duplicated zip file in cloud to the new service object
